...
def page():
    ...
    with col1:
        ...
        fig.update_yaxes(title_text="Satisfaction (/5)", secondary_y=True, range=[0, 5])
        st.plotly_chart(fig, use_container_width=True)

    with col2:
        st.markdown('<div class="section-title">L\'app mobile génère 44 % des contacts mais résout le moins vite</div>', unsafe_allow_html=True)
        ...
        fig2.add_trace(go.Bar(
            ...
            y=channel["contact_volume_k"],
            name="Volume contacts (k)",
            marker_color=[RED if c == "Application mobile" else BLUE for c in channel["service_channel"]],
            text=[f"{v}k" for v in channel["contact_volume_k"]],
            textposition="outside"
        ))
        fig2.update_layout(
